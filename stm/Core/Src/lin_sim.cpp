...
void solve_step(float* states,flight_path *str_h){
    ...
    for(int i=0;i<4;i++){
        lsh.xd_long[i]+=b_long_out[i]; // Calculated on 2 steps ax +bu, this is bu
    }
...
void h_calculation(float state[9], flight_path *str_h){
    str_h->alpha=atan2(state[2],state[0]);
    str_h->v_tot=hypot(state[0],state[2]);
    str_h->beta=atan2(state[1],str_h->v_tot);
